...
describe('telnyx smoke test cases', () => {
  ...
  it('clicking pricing -> voice AI', () => {
    cy.visit('/')
    cy.contains('Согласиться с использованием всех файлов Cookie', { matchCase: false }).click()
  ...
  it('full stack animation check', { scrollBehavior: 'top' }, () => {
    ...
    cy.contains('THE FULL STACK', { timeout: 15000 }).should('be.visible')
  })
  ...
  it('on AI agent choose model "Kimi K2.5", send message "Hello"', () => {
    ...
    cy.contains('Согласиться с использованием всех файлов Cookie', { matchCase: false }).click()
    cy.contains('EXPLORE THE STACK', { matchCase: false }).click()
    cy.contains('CONFIGURE AGENT', { matchCase: false }).click()
  ...
  it('LinkedIn link', () => {
    cy.visit('/')
    cy.contains('Согласиться', { matchCase: false }).click({ force: true })
    cy.get('footer').scrollIntoView()
